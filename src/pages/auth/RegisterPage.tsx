...
import { securityRequestsService } from '../../services/security-requests.service';
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (role === 'security') {
        const statusResp = await securityRequestsService.getStatus(email);

        if (statusResp.status !== 'approved') {
          if (statusResp.status === 'pending') {
            setSuccess(true);
            setError('Your security registration request is pending admin approval. Please try again after approval.');
            return;
          }

          if (statusResp.status === 'rejected') {
            setError('Your security registration request was rejected. Please contact the admin.');
            return;
          }
          ...
          await securityRequestsService.createRequest({ email, location });
          setSuccess(true);
          setError('Security registration request submitted. Please wait for admin approval before signing up.');
          return;
        }
      }
      ...
      const { data, error: signUpError } = await supabase.auth.signUp({
        ...
        password,
        options: {
          emailRedirectTo: `${window.location.origin}/verify-complete`,
          data: {
